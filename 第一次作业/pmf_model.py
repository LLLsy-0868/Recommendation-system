...
class PMF():
    ...
    def __init__(self, R, lambda_alpha=1e-2, lambda_beta=1e-2, latent_size=50, momuntum=0.8,
        ...
        self.U = np.random.normal(0, 0.1, (np.size(R, 0), latent_size))#0.1*self.random_state.rand(np.size(R, 0), latent_size)
        self.V = np.random.normal(0, 0.1, (np.size(R, 1), latent_size))#0.1*self.random_state.rand(np.size(R, 1), latent_size)
    ...
    def train(self, train_data=None, vali_data=None):
        ...
        # print("momuntum_u:",momuntum_u.shape)
        momuntum_v = np.zeros(self.V.shape)
        # print("momuntum_v:",momuntum_v.shape)
        num =0
        for it in range(self.iterations):
            ...
            # momuntum_u = self.lr * grads_u
            # momuntum_v = self.lr * grads_v
            ...
            # train_rmse = RMSE(train_data[:, 2], train_preds)
            # train_rmse_list.append(train_rmse)

            vali_preds = self.predict(vali_data)
            vali_rmse = self.RMSE(vali_data[:, 2], vali_preds)
